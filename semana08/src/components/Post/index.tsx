...
export default function Post({ post, setPost }: PostProps) {
  ...
  async function handleDeleteComment(event: MouseEvent, id: string) {
    ...
    const commmentsFilter = post.comments.filter((comment) => comment.id !== id);
    await axios.patch(`http://localhost:3001/posts/${post.id}`, {
      "comments": commmentsFilter,
    });
    loadPost();
  }
  ...
  return (
    <article className="post">
      ...
      {post.comments?.length &&
        post.comments.map((item) => <Comment key={item.id} comment={item} handleDelete = {handleDeleteComment} handleLike = {handleLikeComment}/>) }
    </article>
  );
}
